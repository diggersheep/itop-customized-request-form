...
abstract class Template extends cmdbAbstractObject
{
	 ...
	 * @return array
	 */
	static public function PopulateUserDataForm(DBObject $oHostObject, \Combodo\iTop\Form\Form $oForm, $aTemplateData, $aValues)
	{
	 ...
	 */
	static protected function MakeFormField(DBObject $oHostObject, $aFieldData, \Combodo\iTop\Form\Form $oForm)
	{
		switch ($aFieldData['input_type'])
		{
			case 'date':
	...
	public function BuildForm(DBObject $oHostObject, $sFormId)
	{
		...
		else
		{
			...
			else
			{
				...
				// Forcing $aUserData to an empty array if the template has changed and the module parameter is set
				// This is used to not keep values between fields of templates with same codes
				...
				if (!is_null($aTemplateData))
				{
					...
					$aExtKeyFields = call_user_func($aCallSpec, $oHostObject, $oUserDataField->GetForm(), $aTemplateData, $aUserData);
				}
				else
				{
					$aExtKeyFields = array();
